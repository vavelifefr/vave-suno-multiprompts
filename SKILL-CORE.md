...
- Single-emission rule: the final 5-tag block appears exactly ONCE in the conversation — never two readable copies. No draft chunk plus final chunk; no implicit parser re-emit beside the package; and when the archive write surfaces its file content (harnesses that echo writes), that write IS the single render — the answer must NOT paste the block again (summaries, counters, Self-check stay outside). On harnesses that hide file content, the answer carries the block once and the archive is written silently. Re-emit only on a fresh explicit trigger, as a replacement.
