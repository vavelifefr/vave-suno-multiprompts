...
4. **Track source** (what the track will be — where the style comes from): `Reference` (one or several) / `Own style — detailed` / `Own style — brief`.
   - On `Reference`, the track name(s) MUST be typed by the operator. A chooser UI cannot attach free text to a choice, so present the name as its **own free-text field immediately after Q4** (`Reference name(s): …`, e.g. `Artist — Track`); the `Reference` choice **without a typed name is incomplete**. If it is still empty, ask once before Q5 — never advance to the package without it. Do NOT proceed straight to Q5 on the bare choice.
   - Then the skill builds the dossier **model-first** (own knowledge), googling only if the result is thin (ADAPTERS A7). `Own style — detailed`: you describe the style yourself in detail; `Own style — brief`: you give a short style and the skill expands it into a detailed Suno-ready style.
5. **Song task** (what to make — this is NOT the language choice). **Conditional on Q4:**
   - Q4 = `Reference` → the **first** option is `As in the reference` (the song mirrors the reference's theme and form; the words stay fully original under R1); the **remaining options stay available** — a theme plus how much text (`little` / `medium` / `a lot`) or the full text pasted as-is, `Instrumental + Hooks and Chops`, `Instrumental` — in case the operator wants an instrumental or a different text volume.
   - Q4 = `Own style — detailed` / `Own style — brief` → own task: a theme plus how much text (`little` / `medium` / `a lot`) or the full text pasted as-is, `Instrumental + Hooks and Chops`, or `Instrumental` (no `As in the reference` option).
